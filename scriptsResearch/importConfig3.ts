...
const {
    ...
    for (const [key, value] of Object.entries(oracleConfig)) {
        ...
        console.log("key", typeof key, typeof value)
        if (key === "signers")
        {
            continue;
        }
        await actMethod(key, value, config, dataStore, false)
    }

    console.log("=== Markets Config ===")
    // const ethUsdMarketCreate = await marketFactory.createMarket(
    //     await weth1.getAddress(),
    //     await weth1.getAddress(),
    //     await usdc1.getAddress(),
    //     DEFAULT_MARKET_TYPE
    //     ) 
    // await ethUsdMarketCreate.wait()

    // const ethUsdSpotOnlyMarketCreate = await marketFactory.createMarket(
    //     ethers.ZeroAddress,
    //     await weth1.getAddress(),
    //     await usdc1.getAddress(),
    //     DEFAULT_MARKET_TYPE
    //     )
    // await ethUsdSpotOnlyMarketCreate.wait()
    ...
    for (const [key, value] of Object.entries(markets)) {
        ...
        if (key === "0" || key === "2") {
            for (const [key1, value1] of Object.entries(value)) {
                ...
                await actMethod(key1, value1, config, dataStore, true, ethUsdMarket)
                await actMethod(key1, value1, config, dataStore, true, ethUsdSpotOnlyMarket)
            }
        }
    }
...
//! npx hardhat run scriptsResearch/importConfig3.ts --network localhost  > outputImportConfig3.log 2>&1 
...
function getMethod(key: any, value: any) {
    ...
    let convertedValue = value;
        if (typeof value === 'number' || typeof value === 'bigint' ) { 
            method = "setUint"
            convertedValue = convertConfig(value)
...
async function actMethod(
  ...
  if (isMarketConfig) {
    ...
    if (fullKey !== undefined && method !== undefined) {
      await addToDataStore(fullKey, value, dataStore, method)
    }
    return;
  }
  if (convertedKey !== undefined && method !== undefined) {
    await addToConfigOrDataStore(convertedKey, convertedValue, dataStore, config, method)
  }
}

async function addToDataStore(fullKey: string, value: any, dataStore: any, method: string) {
  if (method === "setUint") {
    await dataStore.setUint(fullKey, value)
  }
  if (method === "setBytes32") {
    await dataStore.setBytes32(fullKey, value)
  }
  if (method === "setAddress") {
    await dataStore.setAddress(fullKey, value)
  }
  if (method === "setBool") {
    await dataStore.setBool(fullKey, value)
  }
  if (method === "setString") {
    await dataStore.setString(fullKey, value)
  }
}
async function addToConfigOrDataStore(
    convertedKey: string, 
    convertedValue: any, 
    dataStore: any,
    config: any,
