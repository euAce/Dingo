...
const {
    ...
    for (const [key, value] of Object.entries(markets)) {
        if (key === "0" || key === "2") {
            for (const [key1, value1] of Object.entries(value)) {
                ...
                actMethod(key1, value1, config, dataStore, true, value)
            }
        }
    ...
    for (const [key, value] of Object.entries(configGeneral)) {
        actMethod(key, value, config, dataStore, false, null)
...
async function actMethod(
  ...
  if (convertedKey !== undefined && method !== undefined) {
    addToConfigOrDataStore(convertedKey, convertedValue, config, dataStore, method)
  }
}
