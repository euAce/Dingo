...
export type RolesConfig = {
    [role: string]: { [account: string]: boolean; }[];
}[];
...
export default async function (hre: HardhatRuntimeEnvironment): Promise<RolesConfig> {
  const deployer = "70997970c51812dc3a010c7d01b50e0d17dc79c8"; // await hre.getNamedAccounts();
  ...
  if (!(networkName in config)) {
    throw new Error(`Config not found for network ${networkName}`);
  } 
return (config as { [key: string]: RolesConfig })[hre.network.name];
} 
