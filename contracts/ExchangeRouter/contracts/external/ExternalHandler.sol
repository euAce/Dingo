...
import {Errors as ExternalErrors} from "../error/Errors.sol";
...
contract ExternalHandler is IExternalHandler, ReentrancyGuard {
    ...
    function makeExternalCalls(
        ...
        if (targets.length != dataList.length) {
            revert ExternalErrors.InvalidExternalCallInput(targets.length, dataList.length); 
        }

        if (refundTokens.length != refundReceivers.length) {
            revert ExternalErrors.InvalidExternalReceiversInput(refundTokens.length, refundReceivers.length);
        }
    ...
    function _makeExternalCall(
        ...
        if (!Address.isContract(target)) { 
            revert ExternalErrors.InvalidExternalCallTarget(target);
        }

        (bool success, bytes memory returndata) = target.call(data);

        if (!success) {
            revert ExternalErrors.ExternalCallFailed(returndata);
        }
    }
}
