...
library PositionPricingUtils {
    ...
    function getPositionFees(
        ...
        fees.amounts.feeAmountForPool = 
            fees.amounts.positionFeeAmountForPool +
        ...
        fees.amounts.feeReceiverAmount +=
            fees.borrowing.borrowingFeeAmountForFeeReceiver +
            fees.liquidation.liquidationFeeAmountForFeeReceiver;
        ...
        fees.amounts.totalCostAmountExcludingFunding =
            fees.amounts.positionFeeAmount
            + fees.borrowing.borrowingFeeAmount
            + fees.liquidation.liquidationFeeAmount
            + fees.ui.uiFeeAmount
            - fees.amounts.totalDiscountAmount;

        fees.amounts.totalCostAmount =
            fees.amounts.totalCostAmountExcludingFunding
    ...
    function getPositionFeesAfterReferral(
        ...
        fees.amounts.positionFeeFactor = dataStore.getUint(Keys.positionFeeFactorKey(market, forPositiveImpact));
        fees.amounts.positionFeeAmount = Precision.applyFactor(sizeDeltaUsd, fees.amounts.positionFeeFactor) / collateralTokenPrice.min;
        ...
        if (fees.pro.traderTier > 0) {
            ...
            if (fees.pro.traderDiscountFactor > 0) {
                fees.pro.traderDiscountAmount = Precision.applyFactor(fees.amounts.positionFeeAmount, fees.pro.traderDiscountFactor); 
            }
        }
        ...
        if (fees.referral.referralCode != bytes32(0)) {
            ...
            fees.referral.affiliateRewardAmount = Precision.applyFactor(fees.amounts.positionFeeAmount, fees.referral.adjustedAffiliateRewardFactor);
            fees.referral.traderDiscountAmount = Precision.applyFactor(fees.amounts.positionFeeAmount, fees.referral.traderDiscountFactor);
            fees.referral.totalRebateAmount = fees.referral.affiliateRewardAmount + fees.referral.traderDiscountAmount;
        }

        fees.amounts.totalDiscountAmount = fees.pro.traderDiscountAmount > fees.referral.traderDiscountAmount
            ? fees.pro.traderDiscountAmount
            : fees.referral.traderDiscountAmount; 
        fees.amounts.protocolFeeAmount = fees.amounts.positionFeeAmount - fees.referral.affiliateRewardAmount - fees.amounts.totalDiscountAmount;

        fees.amounts.positionFeeReceiverFactor = dataStore.getUint(Keys.POSITION_FEE_RECEIVER_FACTOR);
        fees.amounts.feeReceiverAmount = Precision.applyFactor(fees.amounts.protocolFeeAmount, fees.amounts.positionFeeReceiverFactor);
        fees.amounts.positionFeeAmountForPool = fees.amounts.protocolFeeAmount - fees.amounts.feeReceiverAmount;
